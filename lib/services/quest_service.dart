...
class TaskService {
  static final _chapterWrites = SerialQueue();
  ...
  static const int _questMigrationVersion =
      3; // v3: added quest history for anti-repetition
  ...
  Future<void> _runQuestMigrationIfNeeded() async {
    try {
      ...
      if (kDebugMode) {
        debugPrint(
            '[TaskService] Quest migration v$_questMigrationVersion: forcing daily/nightly quest regeneration');
      }
    } catch (e) {
  ...
  Future<void> _saveWeeklyQuestHistory(
  ...
  Future<void> _saveGeneratedDailyQuestIds(
      String todayKey, List<String> questIds) async {
    try {
      await _storage.save(
          _generatedDailyQuestsKey,
          jsonEncode({
            'dateKey': todayKey,
            'questIds': questIds,
          }));
  ...
  Future<void> _saveGeneratedWeeklyQuestIds(
      String weekKey, List<String> questIds) async {
    try {
      await _storage.save(
          _generatedWeeklyQuestsKey,
          jsonEncode({
            'weekKey': weekKey,
            'questIds': questIds,
          }));
      ...
      final sanitized = quests.map((q) {
        ...
        if (updated.title.isEmpty || updated.description.isEmpty) {
          ...
          updated = updated.copyWith(
            ...
            description:
                updated.description.isEmpty ? 'No description provided' : null,
          );
          changed = true;
        }
        return updated;
  ...
  Future<void> addQuest(TaskModel quest) async {
    try {
      ...
      if (exists) {
        debugPrint(
            'addQuest skipped: quest with id ${quest.id} already exists');
        return;
      }
      quests.add(quest);
  ...
  Future<List<TaskModel>> getActiveQuests() async {
    ...
    return quests
        .where((q) => q.status == 'not_started' || q.status == 'in_progress')
        .toList();
  }

  Future<List<TaskModel>> getQuestsByCategory(String category,
      {bool includeCompleted = false}) async {
    final quests =
        includeCompleted ? await getAllQuests() : await getActiveQuests();
  ...
  Future<void> createDailyQuests() async {
    ...
    if (kDebugMode) {
      debugPrint(
          '[TaskService] createDailyQuests: todayKey=$todayKey, lastGen=$lastGen');
    }
    ...
    if (lastGen == todayKey) {
      if (kDebugMode) {
        debugPrint(
            '[TaskService] Daily quests already generated for $todayKey - loading existing');
      }
      return;
    }
    ...
    quests.removeWhere((q) =>
        ((q.category == 'daily' || q.isDaily || q.type == 'daily') &&
            q.status != 'completed') ||
        ((q.category == 'nightly' || q.type == 'nightly') &&
            q.status != 'completed'));
    ...
    final usedTitles = [
      ...newDailyQuests.map((q) => q.title),
      ...nightly.map((q) => q.title)
    ];
    final history = _loadDailyQuestHistory();
    ...
    if (kDebugMode) {
      debugPrint(
          '[TaskService] Generated ${newDailyQuests.length} daily + ${nightly.length} nightly quests for $todayKey');
      debugPrint(
          '[TaskService] Daily quest titles: ${newDailyQuests.map((q) => q.title).toList()}');
  ...
  Future<void> createWeeklyQuests() async {
    ...
    if (kDebugMode) {
      debugPrint(
          '[TaskService] createWeeklyQuests: weekKey=$weekKey, lastGen=$last');
    }

    if (last == weekKey) {
      if (kDebugMode) {
        debugPrint(
            '[TaskService] Weekly quests already generated for $weekKey - loading existing');
      }
      return;
    }
    ...
    quests.removeWhere((q) =>
        (q.category == 'weekly' || q.isWeekly || q.type == 'weekly') &&
        q.status != 'completed');
    ...
    // Save quest IDs for stable loading
    await _saveGeneratedWeeklyQuestIds(
        weekKey, weekly.map((q) => q.id).toList());
    ...
    if (kDebugMode) {
      debugPrint(
          '[TaskService] Generated ${weekly.length} weekly quests for $weekKey');
  ...
  Future<void> markQuestClaimed(String questId) => _chapterWrites.run(() async {
        try {
          final quests = await getAllQuests();
  ...
  Future<void> ensureBookQuestsForBook(String book, {int? chapter}) async {
    try {
      ...
      final exists = quests.any((q) =>
          (q.category == 'book' || q.type == 'book') &&
      ...
      final q = TaskModel(
        ...
        xpReward: 25,
        rewards: const [
          Reward(type: RewardTypes.xp, amount: 25, label: '25 XP')
        ],
        startDate: now,
  ...
  Future<void> creditChapter(String questId, String chapterKey) =>
      _chapterWrites.run(() async {
        final quests = await getAllQuests();
        ...
        if (q.isCompleted ||
            q.isExpired ||
            q.creditedChapters.contains(chapterKey)) return;
        final progress = (q.currentProgress + 1).clamp(0, q.targetCount);
        quests[index] = q.copyWith(
            currentProgress: progress,
            creditedChapters: [...q.creditedChapters, chapterKey],
            status: progress >= q.targetCount ? 'completed' : 'in_progress',
            completedAt: progress >= q.targetCount ? DateTime.now() : null,
            lastCompletedAt: progress >= q.targetCount ? DateTime.now() : null,
  ...
  Future<void> updateQuestProgress(String questId, int progress) =>
      _chapterWrites.run(() async {
        ...
        if (index != -1) {
          ...
          if (quest.isCompleted || quest.isExpired || progress <= 0) return;
  ...
  Future<void> startQuest(String questId) => _chapterWrites.run(() async {
        final quests = await getAllQuests();
        final index = quests.indexWhere((q) => q.id == questId);
        if (index != -1) {
          final quest = quests[index];
          if (quest.isCompleted || quest.isExpired) return;
          quests[index] = quest.copyWith(
            status: 'in_progress',
            updatedAt: DateTime.now(),
          );
  ...
  Future<void> completeQuest(String questId) => _chapterWrites.run(() async {
        final quests = await getAllQuests();
        final index = quests.indexWhere((q) => q.id == questId);

        if (index != -1) {
          if (quests[index].isCompleted || quests[index].isExpired) return;
  ...
  List<TaskModel> _generateWeeklyQuests(DateTime weekStart) {
    ...
    return [
      ...
      TaskModel(
        ...
        xpReward: 100,
        rewards: const [
          Reward(type: RewardTypes.xp, amount: 100, label: '100 XP')
        ],
        isWeekly: true,
        difficulty: 'Easy',
        startDate: weekStart,
      ...
      TaskModel(
        ...
        xpReward: 150,
        rewards: const [
          Reward(type: RewardTypes.xp, amount: 150, label: '150 XP')
        ],
        isWeekly: true,
        difficulty: 'Medium',
        startDate: weekStart,
      ...
      TaskModel(
        ...
        xpReward: 120,
        rewards: const [
          Reward(type: RewardTypes.xp, amount: 120, label: '120 XP')
        ],
        isWeekly: true,
        difficulty: 'Medium',
        startDate: weekStart,
      ...
      TaskModel(
        ...
        xpReward: 90,
        rewards: const [
          Reward(type: RewardTypes.xp, amount: 90, label: '90 XP')
        ],
        isWeekly: true,
        difficulty: 'Easy',
        startDate: weekStart,
  ...
  List<TaskModel> _getSampleQuests() {
    ...
    return [
      ...
      TaskModel(
        ...
        isWithOthers: true,
        reflectionPrompt:
            'How did praying for them change how you feel toward them?',
        isAutoTracked: false,
        difficulty: 'Easy',
        targetCount: 1,
        xpReward: 20,
      ...
      TaskModel(
        ...
        spiritualFocus: 'Obedience',
        reflectionPrompt:
            'Commit your plans to the Lord in prayer. When done, tap “Mark Complete.”',
        isAutoTracked: false,
        difficulty: 'Easy',
        targetCount: 1,
        xpReward: 35,
      ...
      TaskModel(
        ...
        description:
            'At your next gathering, share a verse that encouraged you',
        type: 'challenge',
        category: 'event',
        questType: 'community',
        spiritualFocus: 'Community',
...
extension _TaskGenerationHelpers on TaskService {
  ...
  List<TaskModel> _dailyTaskPool(DateTime now, DateTime endOfDay) {
    ...
    TaskModel d({
      ...
      String difficulty = 'Easy',
        ...
        TaskModel(
          ...
          isDaily: true,
          isAutoTracked:
              questType == 'scripture_reading' || questType == 'routine',
    ...
    return [
      d(
          ...
          description:
              'A gentle step in Scripture today. Any book you’re drawn to is wonderful.'),
      d(
          title: 'Open the Bible tab',
          description: 'Simply open the Bible and breathe. No rush.',
          questType: 'routine',
          spiritualFocus: 'Stillness',
          xp: 10),
      d(
          title: 'Complete any 1 quest',
          description:
              'Finish any small quest today—celebrate the step you take.',
          questType: 'meta',
          spiritualFocus: 'Perseverance'),
      d(
          title: 'Reflect on a verse',
          description: 'Write a short reflection or prayer in your journal.',
          questType: 'reflection',
          spiritualFocus: 'Reflection'),
      d(
          title: 'Read from your current focus',
          description:
              'Open a chapter in a book you’re focusing on (or any book).'),
      // New daily prompts (12+)
      d(
          title: 'Read a verse that gives you strength today.',
          description: 'Choose any verse that encourages you.',
          questType: 'scripture_reading',
          spiritualFocus: 'Strength',
          xp: 15),
      d(
          title: 'Thank God for something small but meaningful.',
          description: 'Whisper a short prayer of thanks.',
          questType: 'prayer',
          spiritualFocus: 'Thankfulness',
          xp: 15),
      d(
          title: 'Choose one person to bless with kindness.',
          description: 'Send a kind message or help someone today.',
          questType: 'service',
          spiritualFocus: 'Kindness',
          xp: 20),
      d(
          title: 'Read a Psalm and notice one comforting line.',
          description: 'Let one phrase rest in your heart.',
          questType: 'scripture_reading',
          spiritualFocus: 'Comfort',
          xp: 20),
      d(
          title: 'Pick a Proverb and think about its wisdom.',
          description: 'Ask: how can I live this today?',
          questType: 'scripture_reading',
          spiritualFocus: 'Wisdom',
          xp: 20),
      d(
          title: 'Say a short prayer for peace over your day.',
          description: 'Invite God’s peace into what’s ahead.',
          questType: 'prayer',
          spiritualFocus: 'Peace',
          xp: 15),
      d(
          title: 'Reflect on something God has helped you overcome.',
          description: 'Remember His faithfulness.',
          questType: 'reflection',
          spiritualFocus: 'Remembering',
          xp: 20),
      d(
          title: 'Read a verse about courage.',
          description: 'Let God’s word make you brave.',
          questType: 'scripture_reading',
          spiritualFocus: 'Courage',
          xp: 15),
      d(
          title: 'Take a moment of stillness and breathe deeply.',
          description: 'Slow down for a minute with God.',
          questType: 'prayer',
          spiritualFocus: 'Stillness',
          xp: 10),
      d(
          title: 'Write down one blessing from today.',
          description: 'Capture it in your Journal.',
          questType: 'reflection',
          spiritualFocus: 'Gratitude',
          xp: 20),
      d(
          title: 'Read a teaching of Jesus and reflect for a moment.',
          description: 'Choose any passage in the Gospels.',
          questType: 'scripture_reading',
          spiritualFocus: 'Discipleship',
          xp: 20),
      d(
          title: 'Pray for someone who might be struggling.',
          description: 'Ask God to bring comfort and help.',
          questType: 'prayer',
          spiritualFocus: 'Compassion',
          xp: 20),
      d(
          title: 'Share one encouraging verse with a friend.',
          description: 'A gentle nudge of hope today.',
          questType: 'community',
          spiritualFocus: 'Encouragement',
          xp: 20),
      d(
          title: 'Revisit a bookmarked verse.',
          description: 'Read it again slowly and notice new things.',
          questType: 'scripture_reading',
          spiritualFocus: 'Meditation',
          xp: 15),
      d(
          title: 'Read one parable of Jesus.',
          description: 'Let the story speak to your heart.',
          questType: 'scripture_reading',
          spiritualFocus: 'Learning',
          xp: 20),
    ];
  }
  ...
  List<TaskModel> _nightlyTaskPool(DateTime now, DateTime endOfNight) {
    ...
    return [
      n(
          ...
          description: 'Read any chapter tonight for a gentle check-in.',
          questType: 'scripture_reading',
          spiritualFocus: 'Faithfulness',
          xp: 20),
      n(
          title: 'Reflect on one good moment from today.',
          description: 'Thank God for that gift.',
          questType: 'reflection',
          spiritualFocus: 'Gratitude',
          xp: 20),
      n(
          title: 'Thank God for carrying you through the day.',
          description: 'Offer a short prayer of thanks.',
          questType: 'prayer',
          spiritualFocus: 'Thankfulness',
          xp: 15),
      n(
          title: 'Ask God for rest and peace tonight.',
          description: 'Release worries into His hands.',
          questType: 'prayer',
          spiritualFocus: 'Peace',
          xp: 15),
      // Psalms reading quest: targetBook = 'Psalms' for Start navigation and matching
      n(
          title: 'Read a calming Psalm before bed.',
          description: 'Let Scripture settle your heart.',
          questType: 'scripture_reading',
          targetBook: 'Psalms',
          spiritualFocus: 'Comfort',
          xp: 20),
      n(
          title: 'Release any stress or frustration to God.',
          description: 'Breathe and let go.',
          questType: 'reflection',
          spiritualFocus: 'Surrender',
          xp: 20),
      n(
          title: 'Think of one way you grew today.',
          description: 'Notice progress, however small.',
          questType: 'reflection',
          spiritualFocus: 'Growth',
          xp: 20),
      n(
          title: 'Pray for tomorrow’s challenges.',
          description: 'Ask for wisdom and courage.',
          questType: 'prayer',
          spiritualFocus: 'Courage',
          xp: 15),
      n(
          title: 'Journal a short gratitude note.',
          description: 'Write one line of thanks.',
          questType: 'reflection',
          spiritualFocus: 'Gratitude',
          xp: 20),
      n(
          title: 'Read a verse about God’s protection.',
          description: 'Rest in His care.',
          questType: 'scripture_reading',
          targetBook: 'Psalms',
          spiritualFocus: 'Trust',
          xp: 20),
      n(
          title: 'Ask God to renew your mind tonight.',
          description: 'Invite Him to bring peace.',
          questType: 'prayer',
          spiritualFocus: 'Renewal',
          xp: 15),
      n(
          title: 'Reflect on someone you want to forgive.',
          description: 'Pray for grace to release it.',
          questType: 'reflection',
          spiritualFocus: 'Forgiveness',
          xp: 20),
      n(
          title: 'Think about a moment you felt God near today.',
          description: 'Hold that memory with gratitude.',
          questType: 'reflection',
          spiritualFocus: 'Awareness',
          xp: 20),
    ];
  }

  Future<List<TaskModel>> _ensureReflectionSeeds(
      List<TaskModel> existing) async {
    try {
      final now = DateTime.now();
      bool existsByTitle(String t) => existing
          .any((q) => q.title.trim().toLowerCase() == t.trim().toLowerCase());
      TaskModel r(
              {required String title,
              required String description,
              String questType = 'reflection',
              String focus = 'Reflection',
              int xp = 20}) =>
          TaskModel(
            id: _uuid.v4(),
            title: title,
            ...
            xpReward: xp,
            rewards: [
              Reward(type: RewardTypes.xp, amount: xp, label: '$xp XP')
            ],
            startDate: now,
            endDate: null,
            createdAt: now,
      ...
      final seeds = <TaskModel>[
        r(
            title: 'Journal what God is teaching you this week.',
            description: 'Write a few simple thoughts. ✍️'),
        r(
            title: 'Write down one verse that stands out to you.',
            description: 'Copy it into your Journal.'),
        r(
            title: 'Reflect on a prayer God has answered.',
            description: 'Remember His faithfulness.',
            focus: 'Thankfulness'),
        r(
            title: 'Think about how you’ve grown spiritually.',
            description: 'Notice even small steps.',
            focus: 'Growth'),
        r(
            title: 'Write a short prayer in your journal.',
            description: 'Keep it simple and true.',
            questType: 'prayer',
            focus: 'Prayer',
            xp: 15),
        r(
            title: 'Reflect on a recent challenge and how God met you there.',
            description: 'Name His help.',
            focus: 'Hope'),
        r(
            title: 'Pick a verse and rewrite it in your own words.',
            description: 'Let it speak in your voice.',
            focus: 'Meditation'),
        r(
            title: 'Write a gratitude list of three things.',
            description: 'Thank Him for each gift.',
            focus: 'Gratitude'),
        r(
            title: 'Reflect on a step of obedience you want to take.',
            description: 'What is one gentle next step?',
            focus: 'Obedience'),
        r(
            title: 'Journal your thoughts on a psalm.',
            description: 'Choose any Psalm you like.',
            focus: 'Psalms'),
        r(
            title: 'Write a reflection about someone God put on your heart.',
            description: 'Pray and bless them.',
            focus: 'Love'),
        r(
            title: 'Reflect on today’s Scripture reading.',
  ...
  List<TaskModel> _generateDailyQuestsWithHistory() {
    ...
    var availablePool =
        pool.where((q) => !recentTitles.contains(q.title)).toList();

    // If pool is too small, relax to 2-day history
    if (availablePool.length < 3) {
      if (kDebugMode) {
        debugPrint(
            '[TaskService] Daily pool too small (${availablePool.length}), relaxing to 2-day history');
      }
      final relaxedRecent = _getRecentDailyQuestTitles(lookbackDays: 2);
      availablePool =
          pool.where((q) => !relaxedRecent.contains(q.title)).toList();
    }
    ...
    if (availablePool.isEmpty) {
      if (kDebugMode) {
        debugPrint(
            '[TaskService] Daily pool empty after filtering, using full pool');
      }
      availablePool = pool;
    }
    ...
    if (kDebugMode) {
      debugPrint(
          '[TaskService] _generateDailyQuestsWithHistory: pool=${pool.length}, available=${availablePool.length}, picked=${picked.length}');
      debugPrint(
  ...
  List<TaskModel> _generateNightlyQuestsWithHistory(DateTime now) {
    ...
    if (availablePool.length < 2) {
      if (kDebugMode) {
        debugPrint(
            '[TaskService] Nightly pool too small (${availablePool.length}), relaxing to 2-day history');
      }
      final relaxedRecent = _getRecentDailyQuestTitles(lookbackDays: 2);
      availablePool =
          pool.where((q) => !relaxedRecent.contains(q.title)).toList();
    ...
    if (availablePool.isEmpty) {
      if (kDebugMode) {
        debugPrint(
            '[TaskService] Nightly pool empty after filtering, using full pool');
      }
      availablePool = pool;
    }
    ...
    if (kDebugMode) {
      debugPrint(
          '[TaskService] _generateNightlyQuestsWithHistory: pool=${pool.length}, available=${availablePool.length}, picked=${picked.length}');
    }
  ...
  List<TaskModel> _generateWeeklyQuestsWithHistory(DateTime weekStart) {
    ...
    if (availablePool.length < 3) {
      if (kDebugMode) {
        debugPrint(
            '[TaskService] Weekly pool too small (${availablePool.length}), using full pool');
      }
      availablePool = pool;
    }
    ...
    if (kDebugMode) {
      debugPrint(
          '[TaskService] _generateWeeklyQuestsWithHistory: pool=${pool.length}, available=${availablePool.length}, picked=${picked.length}');
      debugPrint(
  ...
  List<TaskModel> _weeklyTaskPool(
      DateTime weekStart, DateTime end, DateTime now) {
    TaskModel w({
      required String title,
      required String description,
      String questType = 'scripture_reading',
    ...
    return [
      ...
      w(
          title: 'Read on 3 different days',
          description:
              'Return on three separate days this week. Gentle rhythm.',
          questType: 'days_active',
          targetCount: 3,
          difficulty: 'Easy'),
      w(
      ...
      w(
          title: 'Read 3 Psalms this week',
          description:
              'Let the Psalms guide your week with comfort and praise.',
          questType: 'scripture_reading',
          targetCount: 3,
          xp: 80,
          difficulty: 'Easy'),
      w(
          title: 'Journal twice this week',
          description: 'Capture two moments of reflection in your journal.',
          questType: 'reflection',
          targetCount: 2,
          xp: 70,
          difficulty: 'Easy'),
      w(
          title: 'Read 7 chapters this week',
          description: 'A chapter a day keeps the Word near.',
          questType: 'scripture_reading',
          targetCount: 7,
          xp: 150),
      w(
          title: 'Pray for 3 different people',
          description: 'Lift up three people in prayer this week.',
          questType: 'prayer',
          targetCount: 3,
          xp: 75,
          difficulty: 'Easy'),
      w(
          title: 'Complete 3 quests in one day',
          description: 'Have a focused day of spiritual growth.',
          questType: 'meta',
          targetCount: 3,
          xp: 100),
      w(
          title: 'Read from 2 different books',
          description: 'Explore two different books of the Bible this week.',
          questType: 'scripture_reading',
          targetCount: 2,
          xp: 80,
          difficulty: 'Easy'),
